...
bool LoadBackground()
{
    bool ret = g_background.LoadImageFile("img//background-night.png", g_screen);
    return ret;
}
...
int main(int argc, char* argv[])
{
    ...
    //Make menu game 
    int ret_menu = SDLCommonFunc::ShowMenu(g_screen, g_font_MENU, "Play Game", "Exit", "img//MenuGame.png");
    if (ret_menu == 1)
        quit = true;
    ...
    bool ret = g_ground.LoadImageFile("img//floor.png", g_screen);
    if (ret == false)
    {
        return -1;
    }
    // if want floor don't move: g_ground.SetRect(0,GROUND_MAP);


    // use goto in c++
again_label:

    TextFunction text_count_;
    text_count_.setColor(TextFunction::WHITE_TEXT);

    PlayerObject player;
    ret = player.LoadImg("img//yellowbird-midflap.bmp", g_screen);
    ...
    while (!quit)
    {
        ...
        LoadRect::RenderRectange(rectange_size, color_data, g_screen);

        GeometricFormat outlie_size(1, 1, SCREEN_WIDTH - 1, 28);
        ColorData color_data1(255, 255, 255);
        LoadRect::RenderOutline(outlie_size, color_data1, g_screen);
