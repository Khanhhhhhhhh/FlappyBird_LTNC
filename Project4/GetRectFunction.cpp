...
bool GetRectFunction::LoadImg(std::string path, SDL_Renderer* screen)
{
    bool ret = SetUp::LoadImageFile(path, screen);
    return ret;
}

void GetRectFunction::SetPos(const int& xp, const int& yp)
{
    this->SetRect(xp, yp);
}

void GetRectFunction::SetXPos(const int& xp)
{
    SDL_Rect rect = this->GetRect();
    this->SetRect(xp, rect.y);
}

void GetRectFunction::ShowImg(SDL_Renderer* screen)
{
    this->Render(screen);
}

void GetRectFunction::DoRun(UINT& x_val)
{
    this->rect_.x += x_val;
...
void DoubleBlock::DrawBound(SDL_Renderer* des)
{
    ...
    LoadRect::RenderOutline(outlie_size, color_data1, des);
}
